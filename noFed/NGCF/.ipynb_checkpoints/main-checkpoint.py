...
def main(args):
    ...
    loss_loger, pre_loger, rec_loger, ndcg_loger, hit_loger, auc_loger, f1_loger= [], [], [], [], [], [], []
    for epoch in range(args.epoch):
        ...
        if args.verbose > 0:
            perf_str = (
                ...
                "precision=[%.5f, %.5f], hit=[%.5f, %.5f], ndcg=[%.5f, %.5f],AUC=[%.5f],F1=[%.5f]"
                % (
                    epoch,
                    t2 - t1,
                    t3 - t2,
...
#             break
    ...
    final_perf = (
        "Best Iter=[%d]@[%.1f]\trecall=[%s], precision=[%s], hit=[%s], ndcg=[%s], auc=[%s], f1=[%s]"
        % (
            idx,
            time() - t0,
